...
func TestBusinessTargetPreservesNativeEndpoints(t *testing.T) {
	...
	credential, err := driver.Parse([]byte(`{"type":"grok","access_token":"test-access","refresh_token":"test-refresh","id_token":"test-id","account_id":"account-one","email":"owner@example.com","expired":"2030-01-01T00:00:00Z","token_endpoint":"https://auth.x.ai/oauth/token"}`))
	if err != nil {
		t.Fatal(err)
	}
		...
		t.Run(root, func(t *testing.T) {
			...
			for _, op := range []struct {
				...
				{"models", func(ctx context.Context, c subscriptionruntime.Credential, target subscriptionruntime.Target) error {
					...
					return err
				}, []string{"https://cli-chat-proxy.grok.com/v1/models"}},
				{"observation", func(ctx context.Context, c subscriptionruntime.Credential, target subscriptionruntime.Target) error {
					...
					return err
				}, []string{"https://cli-chat-proxy.grok.com/v1/billing", "https://cli-chat-proxy.grok.com/v1/billing?format=credits"}},
				{"refresh official", func(ctx context.Context, c subscriptionruntime.Credential, _ subscriptionruntime.Target) error {
					...
					return err
				}, []string{"https://auth.x.ai/.well-known/openid-configuration"}},
			} {
